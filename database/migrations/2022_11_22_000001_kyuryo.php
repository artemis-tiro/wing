...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('kyuryo', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('mise_id')->comment('店ID');
            $table->integer('therapist_id')->comment('セラピストID');
            $table->string('pouch_name')->nullable()->comment('ポーチ');
            $table->integer('pouch_many')->nullable()->comment('ポーチ金額');
            $table->string('adjust_name')->nullable()->comment('調整金名');
            $table->integer('adjust_many')->nullable()->comment('調整金額');
            $table->datetime('working_day')->nullable()->comment('営業日');
            $table->softDeletes();
            $table->timestamps();
        });
    ...
    {
        Schema::table('kyuryo', function (Blueprint $table) {

            // 作成されたテーブルを削除？
            Schema::dropIfExists('kyuryo');
        });
    }
};
