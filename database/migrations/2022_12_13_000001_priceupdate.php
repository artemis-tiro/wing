...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::table('yoyaku', function (Blueprint $table) {

            // 作成されたテーブル->カラム名->どこに挿入するか->条件(nullを許容するなど)->コメント(必須)
            $table->string('encho_id_list')->after('price_id_list')->nullable()->comment('延長');
            $table->dropColumn('back_id_list');
    ...
    public function down()
    {
        Schema::table('yoyaku', function (Blueprint $table) {
            ...
            $table->dropColumn('encho_id_list');
            $table->string('back_id_list')->after('price_id_list')->nullable()->comment('バック');
        });
    }
};
